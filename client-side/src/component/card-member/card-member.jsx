...
const Card = ({item}) => {
  return (
    <Link to={`/member/${item.id}`} className="w-30 p-2 mb-3" key={item.id}>
      <div className="w-full border border-red-600 rounded-full">
        <img src={`http://localhost:3000/profile/${item.profile}`} className="w-24 h-24 rounded-full object-cover object-top border-red-700 mx-auto"/>
      </div>
